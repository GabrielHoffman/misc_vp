...
The LD filter is very strictt R2 > 0.4.  This means that very few non-zero values are reported and the LD matrix may not be positive definite.  But this makes the operation very fast.
```{r sim2}
library(sparseMVN)
i = seq(1, 100000)

lambda = .1
Sigma = (1-lambda)*M[i,i] + lambda*Diagonal(length(i), 1)

ch = Cholesky(Sigma)

# Simulate z-scores
z = solve(ch, rnorm(nrow(Sigma)))
z = as.numeric(z)
names(z) = paste0("SNP_", 1:p)
