...
ml openssl pandoc
R
rmarkdown::render("single_cell_power_AD.Rmd", output_dir='./', intermediates_dir='./'); 
...
The design is the following: we will have overall 200 individuals which includes: 50 controls, 150 AD and two brain regions per individual. From each individual, we will profile 4000 cells
...
library(pwr)
n_samples = c(CTL = 50, AD=150)
n_cells = 4000
n_cells_max = 10000
prop_diff = list(c(0.001, 0.005), c(0.0001, 0.0005), c(0.00001, 0.0001))
n_cell_types = 30
...
ggplot(res, aes(C, power, color=delta)) + geom_line(size=1.2) + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + xlab("# of cells") + ggtitle("Power to detect changes in cell composition") + ylim(0, 1) + facet_wrap(~Disease) + geom_vline(xintercept=n_cells, color="black", linetype="dashed") + geom_hline(yintercept=0.8, color="red", linetype="dashed") #+ scale_x_log10()
```
...
getPowerCurve = function(n1, n2, n_tests, rsq){
	# for each cell number
	res = lapply( seq(0, 2, length.out=1000), function(delta){
...
# for each f value
res = lapply( seq(0, .3, length.out=500), function(f){
	# for each rsq
	res = lapply( c(.5, .6, .7, .8, .9,1), function(rsq){
...
ggplot(res, aes(f, power, color=rsq_text)) + geom_line(size=2) + theme_bw(14) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ylim(0, 1) + xlab("eQTL variance explained") + scale_color_discrete(bquote(R^2)) + geom_hline(yintercept=0.8, color="red", linetype="dashed")
```
