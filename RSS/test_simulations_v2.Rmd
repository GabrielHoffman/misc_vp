...
rmarkdown::render("test_simulations_v2.Rmd")
...
set.seed(1)
data(varPartData)
info$Age = model.matrix(~0 + Batch, info) %*% rnorm(nlevels(info$Batch)) + rnorm(nrow(info))
...
```{r plot.fixed, fig.height=4, fig.width=9, cache=FALSE}
# Fixed effect model. 
...
```{r est.sigma, fig.width=8, fig.height=16, cache=FALSE}
# Apply Emprical Bayes 
fit_eb = eBayes(fit_lm)
fit_eb_mm = eBayes(fit_mm, robust=TRUE)
...
fig_fixed = plotVarianceEstimates(fit_lm, fit_eb, var_true) + ggtitle("Linear model")

# variances from linear mixed model
fig_mixed = plotVarianceEstimates(fit_mm, fit_eb_mm, var_true) + ggtitle("Linear mixed model")
...
ymax = max(sqrt(df$value))*1.03
fig_MSE = ggplot(df, aes(variable, sqrt(value))) + geom_bar(stat="identity", fill=df$fill) + ggtheme + ylab("Root Mean Squared Error") + scale_y_continuous(expand=c(0, 0), limits=c(0, ymax)) + xlab(bquote(Estimate~of~sigma^2))  + coord_flip()
...
# fit_mm_fmt = variancePartition:::eBayesFMT(fit_mm, info, 'Tissue')
