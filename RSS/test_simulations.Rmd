...
df_chisq_mixture = function(lambda){
  ...
  f = function(value){
     KL.gamma(value/2, 2, shape, scale) #+ 
  # KL.gamma(shape, scale, value/2, 2)
  }
  ...
  fobj = optimize(f, c(min(lambda), 1.1*sum(lambda)))
  df = fobj$minimum
...
# mixture of gamma 
dmix_gamma = function(x, shape, scale){

  res = lapply(seq_len(length(shape)), function(i){
    dgamma(x, shape=shape[i], scale=scale[i])
    })
  res = do.call(rbind, res)

  colSums(res) / length(shape)
}
...
```{r mixture_of_chisq, eval=FALSE}
# simulate weights
lambda = sort(rbeta(300, .5, .5))
...
x = seq(1e-4, max(value), length.out=1000)
plot(density(value, from=0), lwd=2)

# simple SW 1 moment method
# df_sat = sum(lambda)^2 / sum(lambda^2) 
# lines(x, dchisq(x, df_sat), col="blue", lty="dashed")

# MLE for chisq
f = function(df){
  sum(pchisq(value, df, log=TRUE))
}
df.mc = optimize(f, c(1e-4, sum(lambda)))$minimum
lines(x, dchisq(x, df.mc), col="red", lty=2)

# sw approximation
# this approximation works better as sum(lambda) increases
df.sw = df_chisq_mixture( lambda )$df
lines(x, dchisq(x, df.sw), col="green", lwd=2, lty=2)

# gamma approximation
est = fitdistrplus::mledist(value, "gamma")
lines(x, dgamma(x, est$estimate[1], est$estimate[2]), col="orange", lwd=1)


```

```{r chisq, fig.width=12}
...
info = droplevels(info[info$Individual %in% lvl[1:8],])
n_genes = 1000
...
ggplot(df, aes(value)) + geom_density() + ggtheme + xlim(0, NA) + ggtitle("Fixed effect RSS /  true variance") + geom_line(data=df2, aes(x, value), color="red") + xlab(bquote(RSS/sigma^2))
```
...
# compare mixture of chisq to mixture of gamma
plot(density(RSS[i]/var_true[i]))
lines(df2$x, df2$value, col="red")
# lines(x, dmix_gamma(x, rdf.shape[i], rdf.scale[i]), col="blue")

```

df = data.frame(var_true, RSS, df.residual=fit_mm$df.residual, index=seq_len(length(RSS)))

ggplot(df, aes(var_true, RSS, color=index<n_de)) + geom_point(size=.2)
