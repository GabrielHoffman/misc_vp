...
# with(tab, heivr.boot.se(logFC.x, logFC.y, (logFC.x/t.x)^2,(logFC.y/t.y)^2, 50, tol=1e-1))
```
...
run_simulation = function(nmax, ngenes, cor.target, nvalues, hsq){
  ...
  df = lapply( nvalues, function(N){
    ...
    tmp = with(tab, cor.test(logFC.x, logFC.y)) %>% tidy %>% mutate(se = sqrt((1 - estimate^2)/parameter))
    cor.se = tmp$se
...
ngenes = 1000 
cor.target = .8
nvalues = c(seq(50, 100, by=30), seq(120, 500, by=75), seq(520, nmax, by=150))
hsq = 0.6
...
df$offset = sapply(df$Method, function(x) 
                switch(x, heivr = 3, Pearson=6, MOM=9))

ggplot(df, aes(N + offset, pmax(0, pmin(1, cor)), col=Method)) +
