...
figList = lapply( unique(info$tissue), function(Tissue){
  ...
  res$fig + ggtitle( paste0(Tissue, ": (p=", format(wt$p.value, digits=3), ')') )
})
...
There are 4 samples per subject per brain region.  These 4 samples represent 2 disections and 2 10X runs.  So even though there are 108 samples, there are only 9 subjects.
...
fitList = lapply( unique(info$tissue), function(Tissue){
...
names(fitList) = unique(info$tissue)


tabList = lapply( unique(info$tissue), function(Tissue){
