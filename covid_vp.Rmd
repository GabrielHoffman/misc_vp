...
df = resGSA[(Geneset %in% df_gs$Geneset),] # & (Tissue=="PFC")
df$zscore = with(df, qnorm(PValue, lower.tail=FALSE)*sign(delta))

# M = matrix(df$zscore, nrow = length(unique(df$CellType)), 
#   dimnames=list(unique(df$CellType), unique(df$Geneset)), byrow=TRUE)

# hcl = heatmap(M, keep.dendro=TRUE)
...
ggplot(df, aes(CellType, Geneset, fill=zscore)) + geom_tile() + theme(aspect.ratio=ratio, axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_fill_gradient2(name = "Z-score", low = "blue", mid="white", high="red", limits=c(-ylim, ylim)) + facet_wrap(~Tissue)
```
