...
evalDifferentialExpression = function(simObj, info, test = "TimePoint", corrRetain=.9){
	...
	for(v in corrRetain){
		...
		form = switch(test, "TimePoint" = ~ (1|Disease) + (1|Batch) + TimePoint + (1|Individual),
							"Disease" 	= ~ Disease + (1|Batch) + (1|TimePoint) + (1|Individual))
		fitList[[key]] = dream(Y_noise, form, info)
	}

	form = switch(test, "TimePoint" = ~ (1|Disease) + TimePoint + (1|Individual),
						"Disease" 	= ~ Disease + (1|TimePoint) + (1|Individual))
	fitList[['Replicates: Two step']] = dream(Y_list[["remove_batch_full_model"]], form, info)

	if( test == "TimePoint"){
		idx = which(info$Disease == "Case")
		form = ~ (1|Batch) + TimePoint + (1|Individual)
		fitList[['Simple: No controls, no replicates']] = dream(Y[,idx], form, info[idx,])
		
		form = ~ Batch + TimePoint + (1|Individual)
		fitList[['Simple: No controls, no replicates - fixed']] = dream(Y[,idx], form, info[idx,])
	
		idx = (! duplicated(info$SampleID)) & (info$Disease == "Case")
		form = ~ TimePoint + (1|Individual)
		fitList[['Naive: No controls, no replicates, ignore batch']] = dream(Y[,idx], form, info[idx,])
	}else{
		form = ~ (1|Batch) + (1|TimePoint) + Disease + (1|Individual)
		fitList[['Simple']] = dream(Y, form, info)
		
		form = ~ Batch + (1|TimePoint) + Disease + (1|Individual)
		fitList[['Simple: - fixed']] = dream(Y, form, info)
	}
...
resultsAll = lapply( c(2,4,6, 8, 10), function(n_control){
	...
	res = lapply( c(2, 4, 6, 8, 10), function(n_controls_per_batch){
		...
		message(opt$n_control, " ", opt$n_controls_per_batch)
		...
		res = lapply( c(.1, .2, .3, .4, .5), function(alpha){
			...
			fitListTP = evalDifferentialExpression(simObj, info, c(.2, .5, .7, .9), test="TimePoint")
			resTP = evalPerformance( fitListTP, coef='TimePointtime2', positives = opt$tpGenes)
			resTP$test = "TimePoint"

			fitListDisease = evalDifferentialExpression(simObj, info, c(.2, .5, .7, .9), test="Disease")
