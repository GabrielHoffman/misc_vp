...
evalDifferentialExpression = function(simObj, info, Y_list, test = "TimePoint", corrRetain=.9){
	...
	if( test == "TimePoint"){
		...
		fitList[['Naive: No controls, no replicates, ignore batch (fixed)']] = tryCatch({eBayes(dream(Y[,idx], form, info[idx,], quiet=TRUE))}, error = function(e) NULL )
...
resultsAll = lapply( c(seq(0.01, 0.05), .1, .2), function(alpha){
	...
	set.seed(1)
	# opt$n_total = 1200
	opt$n_controls_per_batch = 60
	opt$n_samples_per_batch = 60*2
