...
evalDifferentialExpression = function(simObj, info, corrRetain=.9){
	...
	for(v in corrRetain){
		...
		fitList[[key]] = dream(Y_noise, ~ (1|Disease) + (1|Batch) + TimePoint + (1|Individual), info)
	}
...
opt$distr_Batch = estimate_beta_parameters( .6, .01)
...
fitList = evalDifferentialExpression(simObj, info, c(.2, .5, .7, .9))
