...
opt$n_total = 3072 # total number of experiments
# opt$n_samples_per_batch = 10 # size of each batch
# opt$n_controls_per_batch = 2 # modify this parameter
opt$n_time_points = 2
# total number of experiments: N*n_time_points
...
resultsAll = lapply( c(.2), function(alpha){
	...
	set.seed(1)
	# opt$n_total = 1200
	opt$n_controls_per_batch = 192
	opt$n_samples_per_batch = 192*2
	infoTotal = createInfo( opt )
	simObjTotal = generateSimulations( opt, infoTotal )

	res = lapply( c(1,2,3,4,5, 6, 8, 10, 12, 15), function(n_controls_per_batch){
		opt$n_controls_per_batch = n_controls_per_batch

		res = lapply( 192, function(n_samples_per_batch){
...
```{r plot.results, fig.width=15, cache=FALSE, fig.height=12}
...
ggplot(subset(df, test=="Disease"), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test Disease") + geom_hline(yintercept=aupr.random.Disease, elinetype='dashed')
