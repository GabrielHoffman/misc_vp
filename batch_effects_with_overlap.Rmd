	...
	res = lapply( c(3), function(n_controls_per_batch){
		...
		res = lapply( 6, function(n_samples_per_batch){
			...
			if(  v == round(v) ){
				...
				res$coexprNorm = c(NA)
				res$coexprNorm[res$test=="TimePoint"][h] = cpn
...
ggplot(df_coexprNorm, aes(method, (coexprNorm^2 - p)/(p*(p-1)) , fill=method)) + geom_bar(stat="identity") + theme_bw(15) + theme(aspect.ratio=1, legend.position="none") + coord_flip() + ylab("Mean squared error") + ggtitle("Coexpression reconstruction error") #+ facet_wrap()
