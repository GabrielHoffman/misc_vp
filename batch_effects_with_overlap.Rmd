	...
	res = lapply( c(3), function(n_controls_per_batch){
		...
		res = lapply( 6, function(n_samples_per_batch){
			...
			if(  v == round(v) ){
				...
				res$coexprNorm_remove_batch_full_model = coexprNorm( Y_list[['remove_batch_full_model']] )
				res$coexprNorm_remove_batch_simple = coexprNorm( Y_list[['remove_batch_simple']] )
				res$coexprNorm_Y_oracle = coexprNorm( simObj[['Y_oracle']] )
				res$coexprNorm_Y = coexprNorm( simObj[['Y']] )
			}
			res
		})
		do.call('rbind', res)     
