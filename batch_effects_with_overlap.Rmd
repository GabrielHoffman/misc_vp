...
n_tp = 200 # genes different between time points
...
x = sort((1:n_batches)^2, decreasing=TRUE)
...
fitList[['Oracle']] = dream(Y_clean, ~ (1|Disease) + (1|Batch) + TimePoint + (1|Individual), metadata[idx,])
...
fitList[['Replicates: Two step']] = dream(Y_corrected[rs > 0,], ~ (1|Disease) + (1|Batch) + TimePoint + (1|Individual), metadata[idx,])
# fitList[['Replicates: Two step']] = eBayes(fitList[['Replicates: Two step']])

idx = ! duplicated(info$SampleID)
fitList[['Naive']] = dream(Y[,idx], ~ TimePoint + (1|Individual), info[idx,])
fitList[['Naive']] = eBayes(fitList[['Naive']])


fitList[['Simple']] = dream(Y[,unique_ID_idx], ~ (1|Batch) + TimePoint + (1|Disease)+ (1|Individual), info[unique_ID_idx,])
