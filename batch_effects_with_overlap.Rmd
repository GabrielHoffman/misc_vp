...
resultsAll = lapply( c(2,4,6, 8, 10), function(n_control){
	...
	res = lapply( c(2, 4, 6, 8, 10), function(n_controls_per_batch){
		...
		res = lapply( c(.1, .2, .3, .4, .5), function(alpha){
			...
			fitListTP = evalDifferentialExpression(simObj, info, Y_list, test="TimePoint", corrRetain=c(.2, .5, .7, .9))
			resTP = evalPerformance( fitListTP, coef='TimePointtime2', positives = opt$tpGenes)
			resTP$test = "TimePoint"

			fitListDisease = evalDifferentialExpression(simObj, info, Y_list, test="Disease", corrRetain=c(.2, .5, .7, .9))
