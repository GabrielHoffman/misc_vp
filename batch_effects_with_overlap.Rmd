...
opt$distr_ID = estimate_beta_parameters( .4, .01)
opt$distr_Disease = estimate_beta_parameters( .1, .02)
...
# 
df = resultsAll
df = df[-grep("R\\^2", df$method),]

ggplot(subset(df, test=="TimePoint"), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint")
...
fig = ggplot() + theme_bw() + annotate("text", x=0, y=0, label=bquote("Y ~ x\nY ~ x + z"), size=30) #

fig = ggplot2()

library(hexSticker)
s = sticker( 
          package="mvBIC", p_size=10, s_x=1, s_y=.75, s_width=1, s_height=1,
          h_fill = "dodgerblue", h_color="grey70",
          filename='~/Downloads/baseplot.pdf')



library(hexSticker)
s = sticker( ggplot(),
          package="mvBIC", p_size=10,
          h_fill = "dodgerblue", h_color="grey70",
          filename='~/Downloads/baseplot.pdf')
