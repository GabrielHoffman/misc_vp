...
createInfo = function( opt ){
	...
	n_batches = with(opt, n_cases*n_time_points / (n_samples_per_batch - n_controls_per_batch))
...
evalDifferentialExpression = function(simObj, info, Y_list, test = "TimePoint", corrRetain=.9){
	...
	if( test == "TimePoint"){
		...
		fitList[['Simple: No controls, no replicates (random)']] = dream(Y[,idx], form, info[idx,], quiet=TRUE)

		idx = which(info$Disease == "Case")
		form = ~ Batch + TimePoint + Individual
		fitList[['Simple: No controls, no replicates (fixed)']] = tryCatch({eBayes(dream(Y[,idx], form, info[idx,], quiet=TRUE))}, error = function(e) NULL )
		
		idx = which(info$Disease == "Case")
		form = ~ TimePoint + Individual
		fitList[['Naive: No controls, no replicates, ignore batch (fixed)']] = eBayes(dream(Y[,idx], form, info[idx,], quiet=TRUE))
	}else{
...
opt$n_cases = 40 # total number of individuals
opt$n_control = 2 # number of control individuals
...
df = data.frame(y = t(t(simObj$Y_oracle[1,])), info, stringsAsFactors=FALSE)
y_oracle = df$y
fig1 = plotStratify( y ~ Batch, df, main="Oracle", sort=FALSE, legend=FALSE)

df = data.frame(y = t(t(simObj$Y[1,])), info)
...
df = data.frame(y = t(t(exprResid[1,])), info)
...
res = evalPerformance( fitList[1:4], coef='TimePointtime2', positives = simObj$tpGenes)
...
opt$n_cases = 50 # total number of individuals
# opt$n_control = 10 # number of control individuals
# opt$n_controls_per_batch = 10
opt$n_time_points = 2
# total number of experiments: N*n_time_points
opt$p = 5000
opt$n_de = 2500 # genes different between case and control
opt$n_tp = 2500 # genes different between time points
# opt$n_samples_per_batch = 10
# opt$n_batches = opt$n_cases*opt$n_time_points / opt$n_samples_per_batch
opt$n_batches
opt$corrRetain = .8
...
resultsAll = lapply( c( 2, 4, 8, 12, 20), function(n_control){
	...
	res = lapply( n_control, function(n_controls_per_batch){
		...
		res = lapply( c(0.1, .25, .5), function(alpha){
			opt$alpha = alpha

			message(opt$n_control, " ", opt$n_controls_per_batch, " ", opt$alpha)

			res = lapply( c(4, 8, 15, 30), function(n_samples_per_batch){

				opt$n_samples_per_batch = n_samples_per_batch

				if( opt$n_samples_per_batch >= opt$n_control ){
					return(NULL)
				}
				...
				# message("\tDE Disease")
				# fitListDisease = evalDifferentialExpression(simObj, info, Y_list, test="Disease", corrRetain=c(.6, .7, .8, .9))
...
ggplot(subset(df, test=="TimePoint"), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha + n_samples_per_batch, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint") 	#+ geom_hline(yintercept=aupr.random.TimePoint, linetype='dashed')
dev.off()




ggplot(subset(df, test=="TimePoint"), aes(n_control, FPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha + n_samples_per_batch, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint") + geom_hline(yintercept=0.05, linetype='dashed')


# ggplot(subset(df, test=="Disease"), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test Disease")
#+ geom_hline(yintercept=aupr.random.Disease, linetype='dashed')
