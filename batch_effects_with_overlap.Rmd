...
# total number of experiments: N*n_time_points
opt$p = 400
opt$n_de = 200 # genes different between case and control
opt$n_tp = 200 # genes different between time points
opt$n_samples_per_batch = 4
opt$n_batches = opt$n_cases*opt$n_time_points / opt$n_samples_per_batch
opt$n_batches
opt$corrRetain = .8
...
resultsAll = lapply( c( 2, 10), function(n_control){
	...
	res = lapply( n_control, function(n_controls_per_batch){
		...
		res = lapply( c(0.01, 0.1,.5), function(alpha){
			opt$alpha = alpha
			...
			res = lapply( c(4, 8, 12), function(n_samples_per_batch){
				...
				fitListTP = evalDifferentialExpression(simObj, info, Y_list, test="TimePoint", corrRetain=c(.6, .7, .8, .9))
