...
resultsAll = lapply( c( 2, 4, 8, 12), function(n_control){
	...
	res = lapply( n_control, function(n_controls_per_batch){
		...
		res = lapply( c(0.1, .5), function(alpha){
			opt$alpha = alpha

			message(opt$n_control, " ", opt$n_controls_per_batch, " ", opt$alpha)

			res = lapply( c(15, 30), function(n_samples_per_batch){
...
ggplot(subset(df, test=="TimePoint" & n_controls_per_batch==2), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha + n_samples_per_batch, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint") 	#+ geom_hline(yintercept=aupr.random.TimePoint, linetype='dashed')
dev.off()




ggplot(subset(df, test=="TimePoint"), aes(n_control, FPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint") 
