...
resultsAll = lapply( c(seq(0.01, 0.1, length.out=5)), function(alpha){
...
ggplot(df_coexprNorm, aes(n_control, (coexprNorm^2 - p)/(p*(p-1)), color=method)) + geom_line() + theme_bw(15) + theme(aspect.ratio=1, legend.position="bottom", plot.title = element_text(hjust = 0.5)) + ylab("Mean squared error") + ggtitle("Coexpression reconstruction error") + facet_wrap(~alpha, nrow=1)
```
