...
opt$n_cases = 30 # total number of individuals
opt$n_control = 10 # number of control individuals
opt$n_controls_per_batch = 10
opt$n_time_points = 2
# total number of experiments: N*n_time_points
opt$p = 100
opt$n_de = 50 # genes different between case and control
opt$n_tp = 50 # genes different between time points
opt$n_samples_per_batch = 4
opt$n_batches = opt$n_cases*opt$n_time_points / opt$n_samples_per_batch
opt$n_batches
opt$corrRetain = .8
...
resultsAll = lapply( c( 2, 6, 10), function(n_control){
	opt$n_control = n_control


	res = lapply( c(2, 4, 8), function(n_controls_per_batch){
		...
		res = lapply( c(0.1,.5), function(alpha){
			...
			res = lapply( c(4, 8, 12), function(n_samples_per_batch){
...
ggplot(subset(df, test=="TimePoint"), aes(n_control, AUPR, color=method)) + geom_line(size=2) + facet_wrap(~alpha, nrow=1) + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Test TimePoint") 	#+ geom_hline(yintercept=aupr.random.TimePoint, linetype='dashed')
