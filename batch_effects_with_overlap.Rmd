...
evalDifferentialExpression = function(simObj, info, Y_list, test = "TimePoint", corrRetain=.9){
	...
	# fit1 = fitList[['Replicates - joint model']]

	# idx = info$Disease=='Case'
	# form = ~ Batch + TimePoint + (1|Individual)
	# fit2 = dream(Y[,idx], form, info[idx,], quiet=TRUE)

	# plot(topTable(fit1, coef='TimePointtime2', sort.by="none", number=Inf)$t, 
	# 	topTable(fit2, coef='TimePointtime2', sort.by="none", number=Inf)$t)
