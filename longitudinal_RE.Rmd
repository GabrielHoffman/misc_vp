...
# Plot data
ggplot(df_full, aes(Severity, y, color=Subject, shape=as.character(Time))) + geom_point(size=2) + theme_bw() + theme(aspect.ratio=1)
```
...
However, this model has a lot of parameters and requires almost complete observations.  With only a smaller set, we can't fit this model.  We can get close, but with lower power, using the model: `~ Severity + (0+Severity|Subject)`, even on incomplete data.
